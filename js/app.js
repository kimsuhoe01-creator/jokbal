...
const MENU = [
  ...
  {cat:'oven', img:'images/oven-bossam.jpg', n:{ko:'화덕보쌈',vi:'Ba chỉ đút lò',en:'Oven-Roasted Bossam',zh:'烤炉五花肉'}, prices:[['s',tax8(680000)],['m',tax8(780000)],['l',tax8(880000)]], people:{ko:'소 2인 / 중 2~3인 / 대 3~4인',vi:'Nhỏ 2 người / Vừa 2~3 / Lớn 3~4',en:'S 2 / M 2–3 / L 3–4',zh:'小2人 / 中2~3人 / 大3~4人'}},
  ...
  {cat:'side', img:'images/sauce.jpg', n:{ko:'소스 추가',vi:'Thêm nước sốt',en:'Extra Sauce',zh:'加酱料'}, prices:[['single',tax8(20000)]], options:[{id:'spicy-sauce',ko:'매운소스',vi:'Sốt cay',en:'Spicy Sauce',zh:'辣酱',price:tax8(20000)},{id:'garlic-sauce',ko:'마늘소스',vi:'Sốt tỏi',en:'Garlic Sauce',zh:'蒜蓉酱',price:tax8(20000)},{id:'cold-sauce',ko:'냉채소스',vi:'Sốt mù tạt',en:'Mustard Cold Sauce',zh:'芥末凉拌酱',price:tax8(20000)}]},
  ...
  {cat:'drink', img:'images/sapporo-draft.jpg', n:{ko:'삿포로 생맥주',vi:'Bia tươi Sapporo',en:'Sapporo Draft Beer',zh:'札幌生啤酒'}, alt:{ko:'시원한 생맥주 · 부가세 포함',vi:'Bia tươi mát lạnh · Đã gồm VAT',en:'Ice-cold draft beer · VAT included',zh:'冰爽生啤酒 · 含增值税'}, prices:[['single',66000]]},
...
let navScrollLockedUntil = 0;
let categoryTrackingBound = false;
...
const HALL_GIFT_CHOICES = [
  {id:'free-noodle-platter', kind:'hallGift', icon:'🍜', ko:'쟁반국수', vi:'Mỳ khay', en:'Noodle Platter', zh:'拌面拼盘'},
  {id:'free-gorgonzola-pizza', kind:'hallGift', icon:'🍕', ko:'고르곤졸라피자', vi:'Pizza Gorgonzola', en:'Gorgonzola Pizza', zh:'戈贡佐拉披萨'}
];
...
let pendingHallGiftId = '';

function t(k){ return I18N[lang]?.[k] || I18N.ko[k] || k; }
function ui(k){ return UI[lang]?.[k] || UI.ko[k] || k; }
function isHallGiftEligible(item){
  return ['recommend','oven','main'].includes(item?.cat) && !String(item?.n?.ko || '').includes('미니');
}
function applyText(){
...
function updateStickyMetrics(){
  ...
  const height = topbar ? Math.ceil(topbar.getBoundingClientRect().height) : 94;
  document.documentElement.style.setProperty('--topbar-h', `${height}px`);
}
function stickyOffset(){
  const topbarHeight = document.querySelector('.topbar')?.getBoundingClientRect().height || 0;
  const catHeight = document.getElementById('catNav')?.getBoundingClientRect().height || 0;
  return Math.ceil(topbarHeight + catHeight + 16);
}
function setActiveCategory(cat, centerTopNav = false){
  if(!cat) return;
  activeCat = cat;
  document.querySelectorAll('#catNav button, #sideNav button').forEach(button => {
    const selected = button.dataset.cat === cat;
  ...
  if(centerTopNav){
    ...
    const button = nav?.querySelector(`button[data-cat="${cat}"]`);
    if(nav && button){
...
function scrollToCategory(cat){
  ...
  if(!target) return;
  updateStickyMetrics();
  navScrollLockedUntil = performance.now() + 1050;
  setActiveCategory(cat, true);
  const top = target.getBoundingClientRect().top + window.scrollY - stickyOffset();
  window.scrollTo({top:Math.max(0,top), behavior:'smooth'});
}
function renderCats(){
  const navs = [document.getElementById('catNav'), document.getElementById('sideNav')].filter(Boolean);
  ...
  CATS.forEach(cat => {
    ...
    navs.forEach(nav => {
      ...
      button.setAttribute('aria-controls', 'sec-' + cat);
      button.innerHTML = `<span aria-hidden="true">${CAT_ICONS[cat] || ''}</span><span>${t(cat)}</span>`;
  ...
  setActiveCategory(activeCat || CATS[0]);
}
...
function renderMenu(){
  ...
  CATS.forEach(cat => {
    ...
    heading.innerHTML = `<span aria-hidden="true">${CAT_ICONS[cat] || ''}</span><span>${t(cat)}</span>`;
...
function card(item){
  ...
  const giftEligible = isHallGiftEligible(item) ? `<div class="gift-eligible">${ui('giftEligible')}</div>` : '';
...
function updateActiveCategory(force = false){
  if(!force && performance.now() < navScrollLockedUntil) return;
  const marker = window.scrollY + stickyOffset() + 24;
  let current = CATS.find(cat => document.getElementById('sec-' + cat)) || CATS[0];
  CATS.forEach(cat => {
    const section = document.getElementById('sec-' + cat);
    if(!section) return;
    const sectionTop = section.getBoundingClientRect().top + window.scrollY;
    if(sectionTop <= marker) current = cat;
  });
  setActiveCategory(current);
}
function observeSections(){
  if(!categoryTrackingBound){
    categoryTrackingBound = true;
...
function openOrderSheet(item){
  initCartUI();
  const hasExtraChoice = item.cat === 'recommend' || item.options || isHallGiftEligible(item) || (item.prices && item.prices.length > 1);
  if(!hasExtraChoice){
    addDirectToCart(item);
    return;
  }
  pendingItem = item; pendingSizeIndex = 0; pendingQty = 1; pendingHalf = []; pendingOptionIndex = 0; pendingHallGiftId = '';
  $('#orderTitle').textContent = ui('selectOption');
  $('#orderName').innerHTML = `<strong>${localName(item.n)}</strong>`;
  renderSizeChooser(item); renderHalfChooser(item); renderOptionChooser(item); renderHallGiftChooser(item); renderOrderQty();
...
function renderHallGiftChooser(item){
  const box = document.getElementById('hallGiftChooser');
  if(!box) return;
  box.innerHTML = '';
  if(!isHallGiftEligible(item)) return;
  box.innerHTML = `<div class="gift-chooser-head"><div class="choose-title">🎁 ${ui('chooseFreeGift')}</div><p>${ui('freeGiftHelp')}</p></div><div class="gift-choice-grid"></div>`;
  const grid = box.querySelector('.gift-choice-grid');
  HALL_GIFT_CHOICES.forEach(choice => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = pendingHallGiftId === choice.id ? 'selected' : '';
    button.setAttribute('aria-pressed', pendingHallGiftId === choice.id ? 'true' : 'false');
    button.innerHTML = `<span class="gift-choice-icon" aria-hidden="true">${choice.icon}</span><strong>${localName(choice)}</strong>`;
    button.onclick = () => {
      pendingHallGiftId = choice.id;
      renderHallGiftChooser(item);
    };
...
function addPendingToCart(){
  ...
  if(pendingItem.cat === 'recommend'){
    ...
    options.push(...pendingHalf.map(id => HALF_CHOICES.find(c => c.id === id)).filter(Boolean));
  }
  if(pendingItem.options && pendingItem.cat !== 'recommend'){
    const option = pendingItem.options[pendingOptionIndex];
    if(option){ options.push(option); price = option.price; }
  }
  if(isHallGiftEligible(pendingItem)){
    if(!pendingHallGiftId){ alert(ui('needFreeGift')); return; }
    const gift = HALL_GIFT_CHOICES.find(choice => choice.id === pendingHallGiftId);
    if(gift) options.push(gift);
  }
